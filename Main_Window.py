...
Ui_MainWindow, QtBaseClass = uic.loadUiType("Main_Window_UI.ui")
...
class MainWindow(QMainWindow,Ui_MainWindow):
    ...
    def Graph4_Plot(self):
        ...
        index_of_max = np.argmax(magnitude)
        dominant_frequency = frequencies[index_of_max]
        print(f" max : {dominant_frequency}")
        

        # negativeee
        neg_rep=frequencies -  signal.sampling_rate_freq
        neg_plot=pg.PlotDataItem(neg_rep , magnitude , pen=pg.mkPen('g' , width=2))
        self.graph_4.widget.addItem(neg_plot)
        # postiveee
        pos_rep=frequencies + signal.sampling_rate_freq
        ...
        print(f"signal.maxfrequancy : {signal.maxfrequancy}")
        self.graph_4.widget.setLimits(xMin=-9 * signal.maxfrequancy, xMax=9* signal.maxfrequancy)
